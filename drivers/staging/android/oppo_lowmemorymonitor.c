...
int oppo_lowmemory_detect(struct task_struct *task, int tasksize)
{
	...
	if (tasksize > (totalram_pages >> 1)) {
		...
		if (memleak_print_times++ > 3) {
			pr_info("'%s' (%d) tasksize is %ldkB, kill it\n", task->comm, task->pid, tasksize * (long)(PAGE_SIZE / 1024));
			send_sig(SIGKILL, task, 0);
			memleak_print_times = 0;
			return 1;
		}
	} else if (tasksize > ONETHIRD(totalram_pages)) {
		pr_info("'%s' (%d) tasksize is %ldkB, send sig %d to enable memleak detection\n", task->comm, task->pid, tasksize * (long)(PAGE_SIZE / 1024), SIGBACKTRACE_ENABLE);
