...
** ODM_WT_EDIT
** File: - gf_spi.c
** Description: kernel Source file for fingerprint.
**
** Version: V00-01-00-25
** Date : 2018/06/28
**
** ------------------------------- Revision History: -------------------------------
...
/* -------------------------------------------------------------------- */
#ifdef PROC_NODE
static int fpc1020_proc_show(struct seq_file *m, void *v)
{
    seq_printf(m, "%s\n", "fpc1022");
    return 0;
}

static int fpc1020_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, fpc1020_proc_show, NULL);
}

static const struct file_operations fpc1020_proc_fops = {
    .owner = THIS_MODULE,
    .read  = seq_read,
    .open  = fpc1020_proc_open,
    .release = single_release,
};

static int fpc1020_proc_create_node(struct fpc1020_data *fp_dev)
{
    if (fp_dev->proc_entry == NULL) {
        ...
        if (fp_dev->proc_root == NULL) {
            printk("Create dir %s under /proc error!\n",__func__);
            goto err_out;
        }
#endif /* PROC_DIR */
        fp_dev->proc_entry = proc_create(PROC_NODE, 0666, fp_dev->proc_root, &fpc1020_proc_fops);
        if (fp_dev->proc_entry == NULL) {
            printk("Create entry %s under /proc/ error!\n",__func__);
            goto err_out1;
        }
    }
...
static int fpc1020_proc_init(struct fpc1020_data *fp_dev)
{
    if (fp_dev) {
        fp_dev->proc_root = NULL;
        fp_dev->proc_entry = NULL;
...
static void fpc1020_proc_deinit(struct fpc1020_data *fp_dev)
{
    if (fp_dev) {
	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if(!strncmp(buf, "enable", strlen("enable")) && fpc1020->compatible_enabled != 1){
        ...
        printk("yangyagn requested irq %d\n",gpio_to_irq(fpc1020->irq_gpio));
		/* Request that the interrupt should be wakeable */
		enable_irq_wake(gpio_to_irq(fpc1020->irq_gpio));
		fpc1020->compatible_enabled = 1;
        fpc1020_proc_create_node(fpc1020);
