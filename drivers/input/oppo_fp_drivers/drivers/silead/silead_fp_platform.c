...
static nav_keymap_t keymap[] = {
    ...
    //Chunhua.Zhang@ODM_RH.BSP.FP, add for camera key 2019/04/18
    { NAV_KEY_CLICK,    KEY_LEFTBRACE,   },
    { NAV_KEY_DCLICK,   KEY_LEFTBRACE,   },
    { NAV_KEY_LONGPRESS,KEY_LEFTBRACE,   },
};
...
static int silfp_probe(struct platform_device *spi)
{
    ...
    fp_dev = (struct silfp_data *)kzalloc(sizeof(*fp_dev), GFP_KERNEL);
    if (!fp_dev) {
        return -ENOMEM;
    }
