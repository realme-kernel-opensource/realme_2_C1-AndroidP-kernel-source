		...
		if(oppo_sla_info[i].if_up || oppo_sla_info[i].need_up){
			...
			if(dev) {
				if (dev_isalive(dev) && (stats = dev_get_stats(dev, &temp))){
					...
					if(0 == oppo_sla_info[i].rx_bytes ||
						oppo_sla_info[i].rx_bytes > stats->rx_bytes){

						oppo_sla_info[i].rx_bytes = stats->rx_bytes;
						oppo_sla_info[i].minute_rx_bytes = stats->rx_bytes;
						...
						if(do_calc_minute_speed){
							...
							temp_bytes = stats->rx_bytes - oppo_sla_info[i].minute_rx_bytes;
							oppo_sla_info[i].minute_rx_bytes = stats->rx_bytes;
						...
						temp_bytes = stats->rx_bytes - oppo_sla_info[i].rx_bytes;
						oppo_sla_info[i].rx_bytes = stats->rx_bytes;
						...
						if(temp_speed > DOWNLOAD_SPEED &&
							...
							oppo_sla_info[i].download_flag = 0;
						}
...
static int calc_weight_with_left_speed(void)
{
	...
	if(DOWN_LOAD_FLAG == oppo_sla_info[WLAN_INDEX].download_flag &&
		(oppo_sla_info[WLAN_INDEX].congestion_flag > CONGESTION_LEVEL_NORMAL ||
